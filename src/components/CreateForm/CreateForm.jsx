...
const Form = ({ onSubmit }) => {
    ...
    <form onSubmit={handleSubmit}>
      ...
      {!showOptional && (
        <div className={styles.formButtons}>
          <button type="button" className={styles.optionalButton} onClick={() => setShowOptional(true)} disabled={!isRequiredValid()} title="Debes completar nombre y familia para añadir más información">
            <FaPlus /> Añadir información adicional
          </button>
          <button type="submit" className={styles.sendButton}>
            Añadir mariposa <FaCheck />
          </button>
  ...
  <label htmlFor="input-img">
              ...
              <IoImageOutline /> Imagen
            </label>
            <ImageUpload onUpload={(url) => setFormData({ ...formData, img: url })} img={formData.img} />
            {formErrors.img && <p className={styles.errorMessage}>{formErrors.img}</p>}
          </div>

          <div className={`${styles.formGroup} ${styles['origin-group']}`}>
            <label htmlFor="input-origin">
              <HiOutlineGlobeAmericas /> Origen
            </label>
            <textarea
              id="input-origin"
              name="origin"
              ...
              onBlur={handleBlur}
              placeholder="Ej: Originaria de América del Norte."
              title="¿De dónde procede esta mariposa?"
            />
            {formErrors.origin && <p className={styles.errorMessage}>{formErrors.origin}</p>}
          </div>

          <div className={`${styles.formGroup} ${styles['location-group']}`}>
            <label htmlFor="input-location">
              <TbMapPin2 /> Localización
            </label>
            <textarea
              id="input-location"
              name="location"
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Se distribuye desde Canadá hasta el norte de Sudamérica."
              title="¿Dónde podemos encontrar esta mariposa?"
            />
          ...
          <div className={styles.formGroup}>
            ...
            <textarea
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Alas de color naranja con líneas negras."
              title="¿Cómo se identifica esta mariposa?"
            />
          ...
          <div className={styles.formGroup}>
            ...
            <textarea
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Entre 8,9 y 10,2 cm."
              title="¿Qué medidas tiene?"
            />
            {formErrors.size && <p className={styles.errorMessage}>{formErrors.size}</p>}
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="input-fenology">
              <IoCalendarOutline /> Fenología
            </label>
            <textarea
              id="input-fenology"
              name="fenology"
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Migración en otoño, reproducción en primavera y verano."
              title="¿En qué época del año la podemos encontrar?"
            />
            {formErrors.fenology && <p className={styles.errorMessage}>{formErrors.fenology}</p>}
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="input-cycle">
              <LuHourglass /> Ciclo de vida
            </label>
            <textarea
              id="input-cycle"
              name="cycle"
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Huevo, oruga, crisálida y adulto."
              title="¿Cuál es su ciclo vital?"
            />
            {formErrors.cycle && <p className={styles.errorMessage}>{formErrors.cycle}</p>}
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="input-habitat">
              <HiOutlineHome /> Hábitat
            </label>
            <textarea
              id="input-habitat"
              name="habitat"
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Hábitat diverso abarcando desde bosques a campos de algodoncillo."
              title="¿En qué condiciones se desarrolla?"
            />
            {formErrors.habitat && <p className={styles.errorMessage}>{formErrors.habitat}</p>}
          </div>

          <div className={styles.formGroup}>
            <label htmlFor="input-plants">
              <LuFlower2 /> Plantas visitadas
            </label>
            <textarea
              id="input-plants"
              name="plants"
              ...
              onChange={handleChange}
              onBlur={handleBlur}
              placeholder="Ej: Plantas del género Asclepias."
              title="¿Qué especies vegetales poliniza?"
            />
      ...
      {showOptional && (
        <div className={styles.formButtons}>
          <button type="submit" className={`${styles.sendButton} ${styles.SendButtonEnd}`}>
            Añadir mariposa <FaCheck />
          </button>
